...
const PageBanner = ({ title, subtitle, accentFrom, accentTo, gradientFrom, gradientTo }: PageBannerProps) => {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border/10 bg-card/80">
      <svg className="absolute inset-0 w-full h-full opacity-[0.25]" viewBox="0 0 1000 200" preserveAspectRatio="none">
        {[40, 95, 150, 205, 260, 315, 370, 425, 480, 535, 590, 645, 700, 755, 810, 865, 920].map((x, i) => {
          ...
          return (
            <g key={i}>
              <line x1={x} y1={tops[i] - 15} x2={x} y2={tops[i] + heights[i] + 15} stroke={useFirst ? accentFrom : accentTo} strokeWidth="1.5" opacity="0.5" />
              <rect x={x - 8} y={tops[i]} width="16" height={heights[i]} fill={useFirst ? accentFrom : accentTo} rx="1" opacity="0.4" />
            </g>
          );
        })}
      </svg>
